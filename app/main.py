...
@tf.function()
def train_step(image, content_image, style_image, opt, extractor):
...
def main():
    content_path = input("Content Image Name: ").strip()
    style_path = input("Style Image Name: ").strip()
    ...
    content_image = load_img(get_file_path(content_path))
    style_image = load_img(get_file_path(style_path))
    ...
    for _ in range(epochs):
        for _ in range(steps_per_epoch):
            step += 1
            train_step(image, content_image, style_image, opt, extractor)
    ...
    output_path = get_file_path(f"output-{int(time.time())}.jpg")
    tensor_to_image(image).save(output_path)

    print(f"\nCheck output at: {output_path}")
